...
3. $F(x)$ **右**连续。  $\\lim_{ x \to x_{0}^+ }F(x)=f(x_{0})$
4. $P(x_1 < X \leq x_2) = F(x_2) - F(x_1)$。  
5. $P(X = x) = F(x) - F(x-0)$ ，特别的，若 $F(x)$ 在 $x$ 处连续，则 $P(X = x) = 0$ 。
6. $\forall x_{1}<x_{2}$ ,并且 $F(x)$ 在 $x_1,x_2$ 处连续，则 $P(x_{1}<X\le x_2)=P(x_{1}\le X\le x_2)=P(x_{1}\le X\lt x_2)=P(x_{1}<X< x_2) =F(x_{2})-F(x_{1})$
7. 性质 1，2，3 等价于F (x) 为某个随机变量 X 的分布函数  
