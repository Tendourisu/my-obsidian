...
二维离散随机变量的 **LOTUS 定理** 是单变量情况的自然推广，它允许我们直接通过 **联合概率分布** 计算二元函数 \( g (X, Y) \) 的期望，而无需先求出 \( g (X, Y) \) 的分布。具体形式如下：
...
1. **与单变量 LOTUS 的关系**  
   - 单变量 LOTUS 是二维情况的特例（例如当 \( Y \) 退化为常数时）。
...
### **经典应用场景**
1. **协方差计算**  
   协方差 \( \text{Cov}(X, Y) = E[XY] - E[X]E[Y] \)，其中 \( E[XY] \) 可直接用二维 LOTUS 计算：
   \[
   E[XY] = \sum_i \sum_j x_i y_j \cdot P (X = x_i, Y = y_j)
   \]

2. **独立性简化**  
   若 \( X \) 和 \( Y \) 独立，则联合概率 \( P (X=x_i, Y=y_j) = P (X=x_i) P (Y=y_j) \)，此时：
   \[
   E[g (X) h (Y)] = E[g (X)] \cdot E[h (Y)] \quad (\text{独立性导致期望可分解})
   \]

3. **复杂函数期望**  
   例如计算 \( E[\max (X, Y)] \)，无需先求 \( \max (X, Y) \) 的分布，直接通过二维联合分布求和。
...
### **总结**
二维 LOTUS 定理通过联合分布直接计算复杂函数的期望，**避免繁琐的分布转换**，是处理多维随机变量问题的核心工具。其本质是“无意识地”利用已知分布信息，快速求解统计量。
