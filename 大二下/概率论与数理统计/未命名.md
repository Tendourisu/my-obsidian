...
```markdown
# 第9周讲稿—Poisson分布与Poisson过程

## 一、Poisson分布

### 1. 定义与背景
- **定义**：随机变量 $X$ 服从参数为 $\lambda>0$ 的Poisson分布，记为 $X \sim P(\lambda)$，其概率质量函数为：
  $$
  P(X=k) = \frac{e^{-\lambda}\lambda^k}{k!}, \quad k=0,1,2,\ldots
  $$
- **与Poisson过程的联系**：可视为Poisson过程在时间区间 $ [0,1] $ 内的事件计数分布。
...
- **期望与方差**：
  ...
  $$
- **可加性**：独立Poisson变量之和仍为Poisson分布，参数为各参数之和。
...
### 1. 定义
- **强度为 $\lambda$ 的Poisson过程** $\{N_t, t\geq 0\}$ 满足：
  1. $N_0 = 0$；
...
- **协方差与方差**：
  ...
  $$
- **可加性**：独立Poisson过程之和仍为Poisson过程，强度相加。
- **与二项分布的联系**：
  $$
...
### 示例1：顾客购买行为建模
- **场景**：顾客到达商场人数为Poisson过程 $N_t \sim P(\lambda t)$，每个顾客购买概率为 $p$。
- **结论**：
  - 男性顾客数 $N_t^{(1)} \sim P(p\lambda t)$；
  - 女性顾客数 $N_t^{(2)} \sim P((1-p)\lambda t)$；
  - $N_t^{(1)}$ 与 $N_t^{(2)}$ 相互独立。

### 示例2：复合Poisson过程的期望计算
