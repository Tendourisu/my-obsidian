...
（4）计算用上述两次状态和动作让裁判打分输出： $$\hat{q_t} = q(s_t,a_t;w_{now}) 和 q_{t+1} = q(s_{t+1},\tilde{a}_{t+1};w_{now})$$ 
（5） 计算 TD 目标和 TD 误差： $$\hat{y_t} = r_t + \gamma \cdot \hat{q_{t+1}} 和 \delta_t = \hat{q_t} - \hat{y_t}$$
