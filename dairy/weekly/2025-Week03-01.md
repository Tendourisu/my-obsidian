...
tags:
  - " #周记 "
categories: dairy
date: " 2025-01-13T00:21:33+08:00 "
modify: " 2025-01-13T00:21:33+08:00 "
dir: dairy
share: false
cdate: " 2025-01-13 "
mdate: " 2025-01-13 "
...
await dv.view('zob_config/js/show', {
  ...
  files: dv.pages('"dairy"')
    .where(p => {
      ...
      const weekMatch = dv.current().file.name.match(/(\d{4})-Week(\d{1,2})-(\d{2})/);
  ...
  // Collect links under second-level headings (## LINKS)
  li: ([p, li]) => {
    const [_, year, week, month, day] = p.file.name.match(/^(\d{4})-(\d{1,2})-(\d{2})-(\d{2})-(\d{1})$/);
    const formattedDate = `${year}-${month.padStart(2, '0')}-${day.padStart(2, '0')}`;
    // Create a header with date and file link, followed by the content under each "LINKS" heading
...
where 
    ...
    dateformat(date(file.link), "yyyy-'Week'WW") = dateformat(date(this.file.link), "yyyy-'Week'WW")
